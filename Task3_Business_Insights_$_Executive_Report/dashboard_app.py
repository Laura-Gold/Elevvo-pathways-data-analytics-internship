...
def load_master(url):
    ...
    df["month_name"] = df[ORDER_DATE_COL].dt.strftime("%b")
    return df
...
with tab1:
    ...
    with r1c1:
        ...
        s_df["lat"] = s_df["customer_state"].map(lambda x: STATE_CENTROIDS.get(x, (None,None))[0])
        s_df["lon"] = s_df["customer_state"].map(lambda x: STATE_CENTROIDS.get(x, (None,None))[1])
        s_map = s_df.dropna(subset=["lat","lon"])
        s_map["size"] = (s_map["Sales"]/s_map["Sales"].max())*60 + 6
        fig_state = px.scatter_mapbox(s_map, lat="lat", lon="lon", size="size", hover_name="customer_state",
                                      hover_data={"Sales":":,.0f"}, zoom=3.2, mapbox_style="carto-darkmatter",
    ...
    with r1c2:
        ...
        fig_city = px.scatter_mapbox(city_map, lat="lat", lon="lon", size="size", hover_name="customer_city",
                                     hover_data={"Customers":True}, zoom=3.2, mapbox_style="carto-darkmatter",
                                     color_discrete_sequence=[BAR_COLOR])
